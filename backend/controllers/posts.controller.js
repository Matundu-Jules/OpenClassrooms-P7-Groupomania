...
const {getPostQuery, getAllpostsQuery, modifyPostQuery, deletePostQuery} = require("../queries/posts.queries");
...
exports.createPost = async (req, res, next) => {
    try {
        ...
        res.status(201).json({message: "Nouvelle post crée !"});
    } catch (err) {
        next(err);
    }
};
...
exports.modifyPost = async (req, res, next) => {
    try {
        ...
        res.status(201).json({message: "La modification de la post a bien été effectuer."});
    } catch (err) {
        next(err);
    }
};
